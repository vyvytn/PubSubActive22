...
import sys
sys.path.append("..")
from TestEventGenerator import gen_datetime
...
def test_for_patternmatch_seq(pi, pattern, event, df_received_events):
    if len(df_received_events) == 0 or not df_received_events["eventtype"].str.contains(event["eventtype"]).any():
...
def test_for_patternmatch_complex_seq(pi, goal, pattern, event, received_events):
    ...
    if (len(df_recieved_events) == 0 or not df_recieved_events["eventtype"].str.contains(
            df_event["eventtype"].iloc[0]).any()):
    ...
    if len(df_recieved_events) > 1:
        ...
        if df_recieved_events.loc["C", "timestamp"] < df_value_timestamp["timestamp"].iloc[1]:
